    ...
    public class MyTask : PT
    {
        ...
        public static void Solve()
        {
            ...
            var r = File.ReadLines(GetString(), Encoding.Default)
                ...
                }).Where(e=>e.id==k).GroupBy(e=>e.year).Select(g => g.OrderBy(e=>e.hour).First()).OrderBy(e=>e.hour).ThenBy(e=>e.year).Select(e=>e.hour+" "+e.year+" "+e.month).Show();
            if(r.FirstOrDefault()!=null)
            File.WriteAllLines(GetString(), r.ToArray(), Encoding.Default);
            else
                File.WriteAllText(GetString(), "No data", Encoding.Default);
